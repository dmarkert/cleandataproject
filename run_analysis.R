...
#I took the feature names from features.txt and renamed the std and mean columns i'm importing so that they read slighly better.
...
#activity label names.  using them later to translate the activity id levels from the y_text|train.txt file with their names.
...
#everthing is in the same row order so even without ids, I can just cbind the column and everything comes out right.
...
#importing the training data using the sames methods that I did from the test set.
...
#this is step5.  I'm creating an aggregate list of the data grouped by subject and activity and I'm calculating the mean
#of each of the variables for each group
